...
class Weibo(object):
    def __init__(self,
        ...
        if filter != 0 and filter != 1:
            sys.exit(u'filter should be 0 or 1,re input')
        if not self.is_date(since_date):
            sys.exit(u'since_date should be yyyy-mm-dd,re input')
    ...
    def user_to_mongodb(self):
        ...
        print(u'%s information inserted into mongodb' % self.user['screen_name'])
